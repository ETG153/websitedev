...
if(isset($_POST["id"]) && !empty($_POST["id"])){
  ...
  if(empty($input_image)){
    ...
    if(empty($heading_err) && empty($abstract_err) && empty($post_err)){
        ...
        $sql = "UPDATE posts SET heading=?, abstract=?, post=?, img='$fileNameNew', date = now()  WHERE id=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssi", $param_heading, $param_abstract, $param_post, $param_id);
                ...
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post"enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="abstract"  rows="10" cols="50"class="form-control <?php echo (!empty($abstract_err)) ? 'is-invalid' : ''; ?>"><?php echo $abstract; ?></textarea>
